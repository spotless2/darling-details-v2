...
export default function EditProduct() {
  ...
  const { data: productResponse, isLoading: isLoadingProduct } = useQuery({
    ...
    queryFn: async () => await productService.getProduct(productId),
    enabled: !!productId,
  });
  ...
  useEffect(() => {
    if (productResponse?.data) {
      ...
      setTimeout(() => {
        ...
        setCurrentImage(product.imageUrl || product.thumbnailUrl || "");
    ...
    <AdminLayout>
      ...
      >
        <div className="mb-8">
          ...
          <h1 className="font-display text-charcoal text-3xl">Editează Produs</h1>
        </div>
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="space-y-3">
                ...
                {currentImage && uploadedImages.length === 0 && (
                  <div className="mb-2">
                    <p className="text-xs text-charcoal/40 font-sans mb-2">Imagine curentă:</p>
                    <img
                      src={currentImage}
                      alt="Imagine curentă"
                      className="w-24 h-24 object-cover rounded-sm border border-gray-100"
                    />
                  </div>
                )}
                ...
                >
                  <span className="flex items-center gap-2 px-3 py-1.5 bg-primary/10 text-primary text-xs tracking-wider uppercase font-sans rounded-sm group-hover:bg-primary/20 transition-colors whitespace-nowrap">
                    ...
                    {currentImage ? "Schimbă imaginea" : "Alege fișier"}
                  </span>
