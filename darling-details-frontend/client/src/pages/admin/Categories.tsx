...
export default function Categories() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(currentCategory?.id ? "Categorie actualizată" : "Categorie creată");
    },
    onError: (error) => {
  ...
  return (
    <AdminLayout>
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{currentCategory?.id ? 'Editează categorie' : 'Adaugă categorie'}</DialogTitle>
          </DialogHeader>
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent>
          ...
          <div className="py-4">
            <p>Ești sigur că dorești să ștergi categoria <strong>{currentCategory?.name}</strong>?</p>
            <p className="text-red-500 text-sm mt-2">
              Atenție! Această acțiune va șterge și toate produsele asociate acestei categorii.
            </p>
          </div>
          <DialogFooter>
